...
function updateLoginLogoutButtonState(isLoggedIn) {
  if (isLoggedIn) {
    ...
    playlistToolsContainer?.classList.add('hidden');
...
async function fetchCurrentPlayback() {
    ...
    try {
        ...
        } else { updateProfileStatus({ is_playing_on_spotify: false }); }
...
async function loadProfileAndPlaylists() {
  ...
  try {
    ...
    fetchCurrentPlayback(); playbackCheckInterval = setInterval(fetchCurrentPlayback, 15000);
    ...
    if(lastViewedPlaylistId){
        ...
        if(lastPlaylist) { await loadPlaylistTracks(lastPlaylist.id, lastPlaylist.name); } 
        else { localStorage.removeItem('lastViewedPlaylistId'); }
    }
  } catch (error) { console.error('Error loading profile or playlists:', error); if(playlistsContainer) playlistsContainer.innerHTML = `<p class="text-red-400 text-center col-span-full">${error.message || 'Could not load profile.'}</p>`;
  } finally { hideLoading(); }
}

async function fetchPlaylists() {
  playlists = []; let url = 'https://api.spotify.com/v1/me/playlists?fields=next,items(id,name,images,owner(display_name),public,tracks(total))&limit=50';
  while (url) {
...
async function enrichTracksWithGenres(tracksToEnrich) {
    const tracksNeedingGenres = tracksToEnrich.filter(track => track.genre_text === undefined && track.artist);
    if (tracksNeedingGenres.length === 0) return;
    const genrePromises = tracksNeedingGenres.map(track =>
        fetch(`/api/genres?artist=${encodeURIComponent(track.artist)}&track=${encodeURIComponent(track.name)}`)
            .then(res => res.ok ? res.json() : { tags: [] })
            .then(genreData => ({ uri: track.uri, genre_text: genreData.tags?.slice(0, 2).join(', ') || 'Unknown'}))
            .catch(() => ({ uri: track.uri, genre_text: 'N/A' }))
    );
    try {
    ...
    } catch (error) { console.error("Error fetching some genres:", error); }
}
...
async function loadPlaylistTracks(id, name) {
  ...
  try {
    let allTrackItems = [];
    let url = `https://api.spotify.com/v1/playlists/${id}/tracks?fields=next,items(track(id,uri,name,artists(id,name,external_urls),album(id,name,release_date,images,external_urls),duration_ms,popularity,external_urls))`;
    while (url) {
      const data = await spotifyApiCall(url); 
      ...
      const validItems = data.items
        ...
        .map(t => ({
          ...
          duration_ms: t.track.duration_ms, spotify_url: t.track.external_urls?.spotify, genre_text: undefined 
      }));
      allTrackItems.push(...validItems); url = data.next; 
    }
    originalCurrentTracks = allTrackItems;
    await enrichTracksWithGenres(originalCurrentTracks);
...
function updateProfileStatus(spotifyPlaybackInfo = null) {
    ...
    } else { profileStatusText.textContent = 'Music Curator'; }
}
...
function renderTracks(tracksToRender) {
  ...
  if (tracksToRender.length === 0) {
    ...
    if (filterInput && filterInput.value) { 
        emptyState.querySelector('p:first-of-type').textContent = "No tracks match filter";
        emptyState.querySelector('p:last-of-type').textContent = "Clear filter or try different term.";
    } else {
        emptyState.querySelector('p:first-of-type').textContent = "No tracks to display";
        emptyState.querySelector('p:last-of-type').textContent = "This playlist seems empty.";
    } return;
  }
  ...
  document.querySelectorAll('.remove-track-btn').forEach(b => b.onclick = (e) => {
    ...
    originalCurrentTracks = originalCurrentTracks.filter(t => t.uri !== trackUriToRemove);
...
function applyFilter() {
    ...
    if (!filterText) { displayedTracks = [...originalCurrentTracks]; } 
        ...
        displayedTracks = originalCurrentTracks.filter(track => 
            ...
            track.artists.some(artist => artist.name.toLowerCase().includes(filterText))
        );
    }
    renderTracks(displayedTracks);
}
...
async function showTrackInfoModal(trackId) {
    ...
    try {
        ...
        const originalTrack = originalCurrentTracks.find(t => t.id === trackId);
        if (originalTrack && originalTrack.genre_text && originalTrack.genre_text !== 'Unknown' && originalTrack.genre_text !== 'N/A') {
            originalTrack.genre_text.split(', ').forEach(g => combinedGenres.add(g));
        }
...
if(shufflePlaylistBtn) {
    shufflePlaylistBtn.addEventListener('click', () => {
        ...
        applyFilter(); alert('Playlist shuffled locally! Click "Save Changes" to make it permanent on Spotify.');
    });
}
...
if(removeDuplicatesBtn) { 
    removeDuplicatesBtn.addEventListener('click', () => {
        ...
        if (duplicatesRemovedCount > 0) {
            ...
            applyFilter(); alert(`${duplicatesRemovedCount} duplicate track(s) removed. Save changes to make it permanent.`);
